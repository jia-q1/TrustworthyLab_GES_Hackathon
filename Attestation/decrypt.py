...
def encrypt_aes(encrypted_data):
    key = key[:32].ljust(32, b'\0')
    iv = encrypted_data[:16]  # Extract the IV
    encrypted_data = encrypted_data[16:]
    cipher = AES.new(key, AES.MODE_CBC, iv)
...
key = sys.argv[0]
# subprocess.run([os.path.join(os.path.getcwd(), "DeviceNode", "DeviceNode.sh"), "-r"])
# subprocess.run([os.path.join(os.path.getcwd(), "DeviceNode", "DeviceNode.sh"), "-t", "ras_ak.pub"])
# f = open(os.path.join(os.path.getcwd(), "DeviceNode", s_d_service_content.decrypted)
# key = f.read()
# print(key)
